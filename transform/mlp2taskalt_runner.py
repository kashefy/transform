        ...
        for epoch in xrange(self.training_epochs):
            ...
            for itr_epoch in xrange(self.num_batches_train):
                ...
                if self.do_augment_rot:
                    ...
                    # TODO remove hardcoded init
                    rots = rotation_rad(-60, 60, 15)
                    batch_os_one_hot = dense_to_one_hot(np.zeros((self.batch_size_train,), dtype=int) + (len(rots) / 2),
                                                        len(rots))
            ...
            if self.do_task_recognition:
                if len(result.history) == result.history.maxlen and np.absolute(
                        np.mean(result.history) - result.last) < 1e-5:
                    self.logger.debug("Validation accuracy not changing anymore. Stop iterating.")
                    break
            elif self.do_task_orientation:
                if len(result_orient.history) == result_orient.history.maxlen and np.absolute(
                        np.mean(result_orient.history) - result_orient.last) < 1e-5:
    ...
    def validate(self, sess):
        ...
        for _ in xrange(num_batches_val):
            ...
            if self.do_augment_rot:
                ...
                # TODO remove hardcoded init
                rots = rotation_rad(-60, 60, 15)
                batch_os_one_hot = dense_to_one_hot(np.zeros((self.batch_size_val,), dtype=int) + (len(rots) / 2),
                                                    len(rots))
            _, _, _, _ = sess.run( \
