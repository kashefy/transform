...
def run_autoencoder(args):
    ...
    with tf.Session() as sess:
        ...
        for dim in [256, 128, 64]:
            ...
            print('new vars for optimizer', [v.name for v in vars_new])
            optimizer = tf.train.RMSPropOptimizer(learning_rate).minimize(cost, var_list=[vars_new])
            vars_new = sae.vars_new()
            ...
            print('initializing', [v.name for v in vars_new])
            sess.run(init_op)
            summary_writer = tf.summary.FileWriter('/home/kashefy/models/ae/log_simple_stats', sess.graph)
