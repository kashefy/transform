...
class AERunner(AbstractRunner):
    ...
    def learn(self, sess):
        ...
        for dim in self.stack_dims:
            ...
            self.sae.stack(dim)
            cost = self.sae.cost(name='loss_reconstruction')
            
            vars_new = self.sae.vars_new()
#            logging.debug('new vars for optimizer %s' % [v.name for v in vars_new])
            ...
            # Training cycle
    #        
            self.logger.debug('encoder-1: %s' % sess.run(self.sae.sae[0].w['encoder-1/w'][10,5:10]))
    #        if dim == 128:
...
#    fig.savefig(os.path.join(args.log_dir, run_dir, 'train_layerwise_reconstruct.png'))  
    ...
    def __init__(self, params):
        ...
        n_input = self.data.test.images.shape[-1]
